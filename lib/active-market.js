...
import { readTrades } from "@/lib/trades-store";
...
function findTradeForScope(trades, scope) {
  const marketId = normalizeId(scope?.marketId);
  const eventId = normalizeId(scope?.eventId);

  return trades.find((trade) => {
    const sameEvent = eventId && normalizeId(trade?.event_id) === eventId;
    const sameMarket = marketId && normalizeId(trade?.market_id) === marketId;
    return sameEvent || sameMarket;
  });
}

function hasTradeForScope(trades, scope) {
  return Boolean(findTradeForScope(trades, scope));
}
...
function buildCandidatePayload({ event, market, outcomes, prices, liquidity, volume24hr }) {
  const question = formatQuestion(outcomes, market?.question || event?.title || fallbackMarket.question);

  return {
    id: String(market?.id || event?.id || fallbackMarket.id),
    marketId: String(market?.id || event?.id || fallbackMarket.id),
    eventId: String(event?.id || market?.events?.[0]?.id || market?.id || fallbackMarket.id),
    question,
    ...
    prices,
    source: "polymarket",
    category: getTagLabel(event?.tags),
...
async function fetchActiveEsportsMarketUncached() {
  ...
  try {
    ...
    return candidates.map((candidate) => buildCandidatePayload(candidate));
  } catch {
    return [];
  }
}

const getCachedActiveEsportsMarkets = unstable_cache(fetchActiveEsportsMarketUncached, ["active-esports-market-list"], {
  revalidate: 300,
});
...
export async function getActiveEsportsMarket() {
  ...
  const pinnedTrade = pinnedMarket?.marketId
    ? findTradeForScope(trades, {
        marketId: pinnedMarket.marketId,
        eventId: pinnedMarket.eventId,
      })
    : null;

  if (pinnedMarket?.marketId) {
    try {
      const refreshedPinnedMarket = await getMarketSnapshotById(pinnedMarket.marketId);
      const pinnedTradeSettled = pinnedTrade?.status === "settled";

      if (!pinnedTradeSettled && !hasSnapshotEnded(refreshedPinnedMarket)) {
      ...
      if (!pinnedTradeSettled && !hasSnapshotEnded(pinnedMarket)) {
        return pinnedMarket;
      }
    }
  }

  const candidateMarkets = await getCachedActiveEsportsMarkets();
  const nextMarket = candidateMarkets.find((candidate) => {
    if (!candidate?.marketId || candidate.source !== "polymarket") {
      return false;
    }

    return !hasTradeForScope(trades, {
      marketId: candidate.marketId,
      eventId: candidate.eventId,
    });
  });

  if (nextMarket?.marketId) {
    await writeCurrentMarket(nextMarket);
    return nextMarket;
  }

  if (pinnedMarket?.marketId && pinnedTrade?.status !== "settled" && !hasSnapshotEnded(pinnedMarket)) {
    return pinnedMarket;
  }
